...
dotnet restore -p:FlowPackageVersion=0.0.0-local -p:RestoreAdditionalProjectSources=obj/packages -p:NuGetLockFilePath=obj/package.packages.lock.json -p:RestoreLockedMode=false tests/WebUIToolkit.MVVM.Flow.PackageConsumer
dotnet run -c Release --no-restore -p:FlowPackageVersion=0.0.0-local --project tests/WebUIToolkit.MVVM.Flow.PackageConsumer
```

The temporary package-consumer lock is written below ignored `obj/`, leaving the
checked-in project-reference lock portable and stable. The executable exits zero
and prints one `PASS` line only when all public scenarios behave as expected.
