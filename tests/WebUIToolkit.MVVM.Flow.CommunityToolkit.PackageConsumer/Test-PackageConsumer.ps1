...
$consumer = Join-Path $consumerDirectory "WebUIToolkit.MVVM.Flow.CommunityToolkit.PackageConsumer.csproj"
...
function Assert-PackageModeLock {
    ...
    if ($direct.Count -ne 1 -or $direct[0].Name -ne "WebUIToolkit.MVVM.Flow.CommunityToolkit") {
        throw "The consumer lock must have only WebUIToolkit.MVVM.Flow.CommunityToolkit as a direct package."
    }

    $adapter = $framework.PSObject.Properties["WebUIToolkit.MVVM.Flow.CommunityToolkit"].Value
    $toolkit = $framework.PSObject.Properties["CommunityToolkit.Mvvm"].Value
    $runtime = $framework.PSObject.Properties["WebUIToolkit.MVVM.Flow"].Value
    ...
    if ($runtime.type -ne "Transitive" -or $runtime.resolved -ne "0.0.0-local") {
        throw "WebUIToolkit.MVVM.Flow 0.0.0-local must be supplied transitively by the packed adapter."
    }
}
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot "WebUIToolkit.MVVM.Flow.CommunityToolkit.PackageConsumer.csproj") -Destination $consumerDirectory
...
<configuration>
  <packageSources>
    <clear />
    <add key="isolated-flow-feed" value="$feed" />
    <add key="nuget.org" value="https://api.nuget.org/v3/index.json" protocolVersion="3" />
  </packageSources>
  <packageSourceMapping>
    <packageSource key="isolated-flow-feed">
...
"@ | Set-Content -Encoding UTF8 $config
    ...
    $adapterPackage = Join-Path $feed "WebUIToolkit.MVVM.Flow.CommunityToolkit.0.0.0-local.nupkg"
    ...
    if ($UpdateLock) {
        Invoke-DotNet @("restore", $consumer, "-m:1", "--disable-parallel", "--configfile", $config,
            ...
            "-p:FlowCommunityToolkitPackageVersion=0.0.0-local")
    ...
    Assert-PackageContentHash -LockPath $replayLock -PackageId "WebUIToolkit.MVVM.Flow" -PackagePath $runtimePackage
    Assert-PackageContentHash -LockPath $replayLock -PackageId "WebUIToolkit.MVVM.Flow.CommunityToolkit" -PackagePath $adapterPackage
    ...
    Invoke-DotNet @("run", "-c", $Configuration, "--no-restore",
        "-p:FlowCommunityToolkitPackageVersion=0.0.0-local", "--project", $consumer)
    ...
    if (-not $SkipNativeAot) {
        ...
        Invoke-DotNet @("restore", $consumer, "--runtime", $RuntimeIdentifier, "--configfile", $config,
            ...
            "-p:FlowCommunityToolkitPackageVersion=0.0.0-local", "-p:NuGetLockFilePath=$aotLock")
        Invoke-DotNet @("publish", $consumer, "-c", $Configuration, "--runtime", $RuntimeIdentifier,
            ...
            "-p:IlcTreatWarningsAsErrors=true", "-p:FlowCommunityToolkitPackageVersion=0.0.0-local",
            "-p:NuGetLockFilePath=$aotLock")

        $nativeExecutableName = if ($RuntimeIdentifier.StartsWith('win-', [StringComparison]::OrdinalIgnoreCase)) {
            'WebUIToolkit.MVVM.Flow.CommunityToolkit.PackageConsumer.exe'
        } else {
            'WebUIToolkit.MVVM.Flow.CommunityToolkit.PackageConsumer'
        }
