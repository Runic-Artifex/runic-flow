...
fixture_root="${repository_root}/tests/RunicFlow.RunicToolkit.PackageConsumer"
...
[[ -x "${native_executable}" ]] || {
  echo "Native-AOT package consumer was not produced: ${native_executable}" >&2
  exit 1
}
"${native_executable}"

echo "Runic Flow/Runic Toolkit isolated NativeAOT package consumer passed."
