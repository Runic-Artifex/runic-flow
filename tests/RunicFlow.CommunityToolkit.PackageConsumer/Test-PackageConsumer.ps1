...
$consumer = Join-Path $consumerDirectory "RunicFlow.CommunityToolkit.PackageConsumer.csproj"
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot "RunicFlow.CommunityToolkit.PackageConsumer.csproj") -Destination $consumerDirectory
...
"@ | Set-Content -Encoding UTF8 $config
    ...
    if (-not $SkipNativeAot) {
        ...
        $nativeExecutableName = if ($RuntimeIdentifier.StartsWith('win-', [StringComparison]::OrdinalIgnoreCase)) {
            ...
            'RunicFlow.CommunityToolkit.PackageConsumer'
        }
