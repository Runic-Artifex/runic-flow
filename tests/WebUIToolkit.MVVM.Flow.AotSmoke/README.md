...
```console
dotnet restore tests/WebUIToolkit.MVVM.Flow.AotSmoke
...
dotnet restore -r win-x64 -p:FlowPackageVersion=0.0.0-local -p:RestoreAdditionalProjectSources=../WebUIToolkit.MVVM.Flow.PackageConsumer/obj/packages -p:PublishAot=true -p:PublishTrimmed=true tests/WebUIToolkit.MVVM.Flow.AotSmoke
dotnet publish -c Release -r win-x64 --no-restore -p:FlowPackageVersion=0.0.0-local -p:PublishAot=true -p:PublishTrimmed=true -o tests/WebUIToolkit.MVVM.Flow.AotSmoke/obj/aot-publish tests/WebUIToolkit.MVVM.Flow.AotSmoke
